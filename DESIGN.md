...
- Before building or modifying any UI, read this local `DESIGN.md` from the project root and follow it as the visual source of truth.
- Use the front matter tokens for colors, typography, spacing, radius, shadows, components, and constraints. Do not invent new visual tokens unless a requested UI cannot be expressed with the existing system.
- Preserve the near-black product-focused atmosphere: quiet, technical, dense, premium, and restrained.
- Use `{colors.primary}` only for brand mark, primary CTA, focus ring, and link emphasis. Do not use it as decorative background fill.
- Keep cards and containers on the documented surface ladder, with hairline borders and restrained radius.
- Match component behavior to the documented `components:` entries before creating a new variant.
- For Android/Compose implementation, translate pixel tokens into density-independent values while preserving the same hierarchy and proportions.
- If a request conflicts with this design system, explain the conflict before changing the visual direction.
